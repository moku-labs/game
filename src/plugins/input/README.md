...
| release | with a target: `flow.gate.answer(dropAnswer(...))`, both components read NOW. Then always, in this order: `unmute()`; `settle(entity)` when nothing took it; `lift(entity, false)`; the tags go; `flow.gate.pointer(false)` |
