 ...
 * is on both screens and opens the same popup. `open` rests on the popup; a volume step or a
 * language switch commits and comes back to it, a reset asks first with the confirm stacked on
 * top. It is left with `closed`: by the X, the backdrop, or a confirmed reset.
 *
...
import { confirmReset, open, setLocale, setVolume } from "./nodes";

export const settingsFlow = defineFlow("settingsPopup", {
  nodes: { open, setVolume, setLocale, confirm: confirmReset },
  start: "open",
